...
export const useRutasService = () => {
  ...
  const crearLineaYRuta = async (data: CrearLineaYRutaRequest): Promise<ServiceResult<{ id: number }>> => {
    setLoading(true);
    try {
      ...
      return { success: false, error: res?.error || "No se pudo crear la línea/ruta" };
    } catch (e) {
      setLoading(false);
 ...
 const obtenerRutas = async (params: RutasQueryParams = {}) => {
  ...
  const obtenerRutaPorId = async (id: number): Promise<ServiceResult<RutaFrontend>> => {
    setLoading(true);
    try {
  ...
  const editarLineaYRuta = async (id: number, data: CrearLineaYRutaRequest): Promise<ServiceResult<void>> => {
    setLoading(true);
    try {
      const response = await fetch(`${BASE_URL}/${id}`, {
        method: "PUT",
      ...
      return { success: false, error: res?.error || "No se pudo editar línea/ruta" };
    } catch (e) {
      setLoading(false);
      return { success: false, error: (e as any)?.message || "Error de red" };
    }
  ...
  const eliminarRuta = async (id: number): Promise<ServiceResult<void>> => {
    ...
    try {
      ...
      if (response.ok) return { success: true };
      const res = await response.json().catch(() => ({}));
      return { success: false, error: res?.error || "No se pudo borrar la ruta" };
    } catch (e) {
      setLoading(false);
      return { success: false, error: (e as any)?.message || "Error de red" };
    }
