...
const API = (import.meta as any)?.env?.VITE_API_URL ?? "http://127.0.0.1:4000";

type ServiceResult<T = unknown> = Promise<
...
export const supervisorService = {
  async getTurnos(token?: string): ServiceResult<TurnoAsignado[]> {
    ...
    if (!res.ok)
      return fail(res, "No se pudieron obtener los turnos asignados");
  ...
  async assignTurno(
    ...
    if (!isISODate(payload?.fecha))
      return { success: false, error: "Falta fecha válida (AAAA-MM-DD)" };
  ...
  async updateTurno(
    ...
    const res = await fetch(
      `${API}/supervisor/actualizar-turno/${turnoAsignadoId}`,
      {
        method: "PUT",
        headers: headers(token),
        body,
  ...
  async deleteTurno(
    ...
    const res = await fetch(
      `${API}/supervisor/eliminar-turno/${turnoAsignadoId}`,
      {
        method: "DELETE",
        headers: headers(token),
      }
